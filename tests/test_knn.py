...
def _run(monkeypatch, tmp_path, pcas_tsv, n_neighbors=15, seed=42, flavor="rapids"):
...
def test_knn_cagra_fp16(monkeypatch, tmp_path, pcas_tsv):
    n_neighbors = 15
    out = _run(monkeypatch, tmp_path, pcas_tsv,
               n_neighbors=n_neighbors, flavor="rapids-cagra-fp16")
    graph = read_graph(out)
    n_cells = graph.distances.shape[0]
    assert graph.distances.nnz == n_neighbors * n_cells
